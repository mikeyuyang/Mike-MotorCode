...
// FILE:	mb_crc.c
//
// TITLE:	Sourse file for crc check.
//
// AUTHOR:	
//
// DATE:	
//
// Description: This file include the crc value table and the process for crc calculating.
//
//------------------------------------------------------------------------------------------------------
// Created by: 		
// Created date:  
...
USHORT GenMBCRC16( UCHAR * pFrame, USHORT Len )
{
    ...
    while( Len-- )
    {
        ...
        ucCRCLo = ( UCHAR )( ucCRCHi ^ auchCRCHi[iIndex] );
        ucCRCHi = auchCRCLo[iIndex];
    }
	result = ucCRCHi;
	result = (result << 8) + ucCRCLo;
    return result;
}
